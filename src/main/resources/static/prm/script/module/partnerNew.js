                ...
                if(data.code == 200){
                    if(data.data){
                        ...
                        if(RepeatedPhoneCount>0){
                            var statu = confirm("联系人已存在，是否确认保存？");
                            if(statu) {
                                newConObj.id = addingCon.find('.no2').find('span').text();
                                newConObj.name = addLinkmanName;
                                newConObj.obligation = addLinkmanObl;
...
// function alertShow(domName){
//     $('.alertShow').show().delay(3000).hide(300,function(domName){
//         $('#alertWords').text('');
//         console.log(domName)
//         $('html,body').animate({
//             scrollTop: parseInt($(''+domName).offset().top)-200
//         },300);
//     });
// }